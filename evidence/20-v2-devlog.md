...
**需求（老大）**：新功能不完善点全补齐，兼容性稳定性都要保证。

**改进清单**：
1. **监控心跳**（--monitor）：MonitorSession.startHeartbeat() 每 interval 推送💓心跳，确认监控 agent 存活（值守场景"静默=未知"问题解决）
2. **webhook 推送重试**：指数退避（1s→2s→4s），最多 3 次，HTTP 500/网络抖动不丢告警
3. **hprof 流式解析**：countMagicStream 1MB 分块 + 3 字节重叠边界，大堆（1GB+）不再 OOM/跳过
4. **AiClient 重试**：5xx/429 自动重试 2 次（指数退避），4xx 不重试，网络异常也重试
5. **NetworkAnalyzer Windows 支持**：netstat -ano 按 PID 过滤（此前 Windows 直接落到 analyzeMac 失败）
6. **RuleUpdater 缓存**：远端版本与本地一致 + 本地有规则 → 跳过全量下载（弱网提速）
7. **主程序 usage** 补 --monitor 说明（取证程序支持 webhook 值守监控）

**验证**（全部实测）：
- 心跳推送: 💓 内存马值守监控心跳（目标/状态/时间）✓
- webhook 告警 + dump 正常 ✓
- hprof 流式: 结果与之前一致（305 段字节码）✓
